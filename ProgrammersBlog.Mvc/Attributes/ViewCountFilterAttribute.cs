...
{
    //Bu attributu class ve methodlarda kullanacağımızı belirttik.
    [AttributeUsage(AttributeTargets.Class | AttributeTargets.Method)]
    public class ViewCountFilterAttribute:Attribute,IAsyncActionFilter
    {
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            //attribute kullandığımız aciton' a gelen parametreyi burada alıyoruz.
            var articleId = context.ActionArguments["articleId"];

            if (articleId is not null)
            {
                //article + articleId olucak şekilde cookie alıyoruz. Eğer daha önce oluşmamışsa ilk defa sayfaya giriliyor demek
                //cookiyi oluşturuyoruz.
                string articleValue = context.HttpContext.Request.Cookies[$"article{articleId}"];
                if (string.IsNullOrEmpty(articleValue))
                {
                    //Cookie okuşturma işlemini yapıyoruz Set fonk ile
                    Set($"article{articleId}", articleId.ToString(), 1, context.HttpContext.Response);
                    //Article Service ulaşarak ilk defa sayfaya girildiği için cookie yi oluşturduktan sonra sayfa arttırma işlemini yapıyoruz.
